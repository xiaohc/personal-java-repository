...
> 比如sqrt(x)函数计算x的平方根，只要x不变，不论什么时候调用，调用几次，值都是不变的。  
  ...
  对比 <kbd>命令式编程</kbd> 和 <kbd>函数式编程</kbd>：函数式编程在求解复杂问题时更有优势

> 👉  <kbd>命令式编程</kbd>就像小学方法解数学题，正向思维一步一步算。  
> 👉  <kbd>函数式编程</kbd>就像使用方程式解数学题，使用 y=ax+b 这种函数，用抽象来解决很复杂的问题。

函数式编程范式对比函数式编程范式，其优势在信息隐藏上，在解题时更抽象，更少关心具体步骤，故函数式编程在求解复杂问题时更有优势
p.s. 技术的首要使命是管理软件的复杂度，管理软件复杂度的2种方向：模块化（分类管理） 和 信息隐藏（分层管理）
